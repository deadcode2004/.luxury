...
export default function Features() {
  ...
  return (
    <section className="py-24 bg-surface border-t border-b border-gray-100">
      <div className="container mx-auto px-4 md:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          {features.map((feature, index) => (
            <div key={index} className="text-center group p-8 rounded-2xl hover:bg-background hover:shadow-soft transition-all duration-300 border border-transparent hover:border-gray-100">
              <div className="flex justify-center transform group-hover:-translate-y-2 transition-transform duration-300">
                {feature.icon}
              </div>
              <h3 className="text-2xl font-bold mb-4 text-secondary">{feature.title[language]}</h3>
              <p className="text-gray-500 text-base leading-relaxed">{feature.description[language]}</p>
            </div>
          ))}
        </div>
      </div>
