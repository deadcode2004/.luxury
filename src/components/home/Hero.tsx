...
type Slide = (typeof FALLBACK_SLIDES)[number];

function cmsToSlide(cms: CmsStorefront): Slide {
  return {
    id: 0,
    image: cms.hero.image || FALLBACK_SLIDES[0].image,
    subtitle: cms.hero.subtitle,
    title: cms.hero.heading,
...
export default function Hero() {
  ...
  const [cmsSlide, setCmsSlide] = useState<Slide | null>(null);

  useEffect(() => {
    let cancelled = false;
    fetchPublicCms()
      .then((cms) => {
        if (!cancelled && cms?.hero?.heading?.ar) {
          setCmsSlide(cmsToSlide(cms));
        }
      })
      .catch(() => undefined);
    return () => {
  ...
  const slides = useMemo(() => {
    if (!cmsSlide) return FALLBACK_SLIDES;
    return [cmsSlide, ...FALLBACK_SLIDES.slice(1)];
  }, [cmsSlide]);
      ...
      {slides.map((slide, index) => (
        ...
        >
          <Image
            ...
            priority={index === 0}
            quality={100}
            unoptimized={slide.image.startsWith("/storage/")}
          />
